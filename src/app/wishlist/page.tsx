...
import { Heart, Trash2, ShoppingCart, Loader2, Gift, ArrowRight, Sparkles } from 'lucide-react';
...
export default function WishlistPage() {
  ...
  // Load from localStorage if user is not logged in / is guest
  useEffect(() => {
    if (!authLoading && !user) {
      const savedWishlist = localStorage.getItem('giftart_wishlist');
      if (savedWishlist) {
        try {
          setLocalItems(JSON.parse(savedWishlist));
        } catch (e) {
  ...
  const { data: firebaseItems, loading: firebaseLoading } = useCollection<any>(wishlistQuery);

  const wishlistItems = user ? firebaseItems : localItems;
  const loading = user ? (firebaseLoading || authLoading) : (localLoading || authLoading);
  ...
  const handleRemove = async (docId: string, name: string) => {
    if (user) {
      ...
      try {
        ...
        items = items.filter((item: any) => item.id !== docId);
        localStorage.setItem('giftart_wishlist', JSON.stringify(items));
        setLocalItems(items);
        toast({
          title: "Removed from Wishlist",
          description: `${name} has been removed from guest wishlist.`
        });
      } catch (err) {
        console.error("Error removing guest item:", err);
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 pt-28 lg:pt-36 max-w-6xl">
        {/* Guest Banner */}
        {!user && !loading && (
          <div className="mb-8 bg-gradient-to-r from-violet-50 via-indigo-50 to-sky-50 border border-indigo-100 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-indigo-500 shadow-sm shrink-0">
                <Sparkles className="h-5 w-5 animate-pulse" />
              </div>
              <div className="text-center sm:text-left">
                <h3 className="font-bold text-slate-800 text-sm">Save your wishlist permanently!</h3>
                <p className="text-slate-500 text-xs mt-0.5">Create an account or sign in to access your saved items from any device.</p>
              </div>
            </div>
            <Button asChild className="rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[10px] tracking-widest px-6 h-11 shrink-0 shadow-md shadow-indigo-500/10">
